...
EntityManager.prototype.remove = function ( entityID, componentType) {
    ...
    if (arguments.length === 1) {
        console.log("add to remove list", entityID);
...
EntityManager.prototype.emptyRemoveEntitiesList = function() {
    ...
    for( componentType in entity ) {
        ...
        if (entity.hasOwnProperty(componentType)) {
            ...
            if( this.indexData[componentType] ) {
                ...
                for (var j = this.indexData[componentType].length - 1; j >= 0; j--) {
                    if(this.indexData[componentType][j] === entityID) {
                        this.indexData[componentType].splice(j,1);

                    }
                }

                this.emit('componentRemoved', componentType, entityID );
            }
